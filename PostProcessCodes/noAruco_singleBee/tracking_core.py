...
class TrackingEngine:
    ...
    def run_tracking(self, progress_callback=None):
        ...
        missed = 0
        last_angle_deg = -1.0
        last_filtered_pos: Optional[Point] = None
        ...
        for local_index in range(total_frames):
            ...
            # Detection frame schedule. With detect_every_n_frames = 5, the
            # engine detects on local frame numbers 1, 5, 10, 15, ... . The
            # skipped frames still update the adaptive background below.
            should_detect = (
            ...
            if should_detect:
                ...
                if adaptive_bg is not None:
                    # Only compute the adaptive background image when detection
                    # will actually use it. The rolling buffer is still updated
                    # on every frame, preserving the original background logic.
                    bg_channel = adaptive_bg.get_background()
                    bg_history_count = adaptive_bg.history_count

                previous_position = last_filtered_pos
            ...
            if adaptive_bg is not None:
                # Update AFTER optional detection so the current frame is not
                # part of its own adaptive background. This still runs for
                # skipped frames, so the background history remains continuous.
                adaptive_bg.update(self._crop_frame(frame, adaptive_crop_bbox))

            if result is not None and result.detected:
                raw_pos = (int(round(result.x)), int(round(result.y)))
                raw_ang = float(result.angle_deg)
                self.detection_area_history.append(float(result.area))
                self.candidate_count_history.append(len(result.candidates))
            else:
                # Either detection was skipped for speed, or detection was
                # attempted and no valid bee was found. Raw output remains
                # missing because no real measurement exists on this frame.
                raw_pos = (-1, -1)
                raw_ang = -1.0
                self.detection_area_history.append(0.0)
                self.candidate_count_history.append(0 if result is None else len(result.candidates))

            detection_attempted = result is not None
            detected_now = bool(result is not None and result.detected)
            ...
            if not self.kalman_enabled:
                if detected_now:
                    ...
                    filt_ang = raw_ang
                    pred_flag = 0
                    last_filtered_pos = (float(filt_pos[0]), float(filt_pos[1]))
                    last_angle_deg = raw_ang
                else:
                    filt_pos = (-1, -1)
                    filt_ang = -1.0
                    pred_flag = 0
                    last_filtered_pos = None
            else:
                if detected_now:
                    ...
                    if kf is None:
                        ...
                        fx, fy = kf.update(result.x, result.y)
                    else:
                        # Follow the same predict-then-update pattern as the old
                        # tracking core. This preserves the constant-velocity model.
                        kf.predict()
                        fx, fy = kf.update(result.x, result.y)
                    ...
                    missed = 0
                    last_angle_deg = raw_ang
                    last_filtered_pos = (float(fx), float(fy))
                else:
                    # Skipped frames are not counted as missed detections. This
                    # prevents the track from being reset just because the user
                    # intentionally detects every N frames for speed.
                    should_count_as_missed = detection_attempted
                    if kf is not None and (missed < self.max_missed_frames or not should_count_as_missed):
                        fx, fy = kf.predict()
                        filt_pos = (int(round(fx)), int(round(fy)))
                        filt_ang = float(last_angle_deg)
                        pred_flag = 1
                        ...
                        if should_count_as_missed:
                            missed += 1
                        last_filtered_pos = None
                        if should_count_as_missed and missed > self.max_missed_frames:
                            kf = None

            raw_coords_history.append([raw_pos])
            filtered_coords_history.append([filt_pos])
            self.raw_angles.append([raw_ang])
            self.filtered_angles.append([filt_ang])
            self.filtered_is_prediction.append([pred_flag])
    ...
    def _append_realtime_rows(
        ...
        *,
        ...
        raw_file.write(f"{raw_pos[0]}\t{raw_pos[1]}\t{float(raw_ang):.6f}\n")
        filtered_file.write(f"{filt_pos[0]}\t{filt_pos[1]}\t{float(filt_ang):.6f}\n")
