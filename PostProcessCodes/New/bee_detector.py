...
class BeeDetector:
    ...
    def _compute_cell_means_from_warp(self, warp_gray: np.ndarray) -> np.ndarray:
        gray32 = warp_gray.astype(np.float32)
        integ = cv2.integral(gray32, sdepth=cv2.CV_64F)
        ...
        return sums / np.maximum(self._sample_areas, 1.0)

    def _render_cell_grid_image(self, cell_values: np.ndarray) -> np.ndarray:
        vis = np.zeros((self.side_pixels, self.side_pixels), dtype=np.uint8)
        cell_values_u8 = np.clip(np.rint(cell_values), 0, 255).astype(np.uint8)

        for r in range(self.total_cells):
            y0 = int(round(r * self.side_pixels / self.total_cells))
            y1 = int(round((r + 1) * self.side_pixels / self.total_cells))
            for c in range(self.total_cells):
                x0 = int(round(c * self.side_pixels / self.total_cells))
                x1 = int(round((c + 1) * self.side_pixels / self.total_cells))
                vis[y0:y1, x0:x1] = cell_values_u8[r, c]

        return vis

    def _sample_grid_from_warp(self, warp_gray: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float]:
        cell_means = self._compute_cell_means_from_warp(warp_gray)
        cell_means_u8 = np.clip(np.rint(cell_means), 0, 255).astype(np.uint8)
        otsu_thresh, cell_bw = cv2.threshold(
            cell_means_u8,
            0,
            255,
        ...
        cell_is_white = cell_bw == 255

        bits = np.empty((self.total_cells, self.total_cells), dtype=np.uint8)
        if self.white_bit_value == 1:
            bits[:, :] = cell_is_white.astype(np.uint8)
        else:
            bits[:, :] = (~cell_is_white).astype(np.uint8)

        debug_cell_means = self._render_cell_grid_image(cell_means)
        debug_cell_bw = self._render_cell_grid_image(cell_bw)

        return bits, debug_cell_means, debug_cell_bw, float(otsu_thresh)
    ...
    def _decode_candidate(self, warp_gray: np.ndarray) -> Optional[Dict[str, Any]]:
        full_bits, cell_means_vis, cell_bw_vis, otsu_thresh = self._sample_grid_from_warp(warp_gray)
        ...
        if self.show_debug and (self.debug_marker_id is None or best_id == self.debug_marker_id):
            ...
            print("cell-mean otsu threshold:", f"{otsu_thresh:.2f}")
            print("---")

            warp_vis = self._make_debug_grid_overlay(warp_gray)
            means_vis = self._make_debug_grid_overlay(cell_means_vis)
            bw_vis = self._make_debug_grid_overlay(cell_bw_vis)

            cv2.imshow("debug_warp_gray", warp_vis)
            cv2.imshow("debug_cell_means", means_vis)
