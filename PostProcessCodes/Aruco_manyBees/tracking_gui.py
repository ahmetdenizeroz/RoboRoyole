...
class MainWindow(QtWidgets.QWidget):
    ...
    def show_scrollable_help(self):
        help_text = """
        <h2>Single-Bee Tracking GUI Help</h2>
        <h3>1. Setup</h3>
        <ul>
        <li><b>Load Video:</b> Select the experiment video.</li>
        <li><b>Open Settings:</b> Select the single-bee detector settings TXT file saved by the tuner GUI.</li>
        <li><b>Build BG:</b> Optional. Only needed when the selected settings preset uses <code>background_mode = static</code>.</li>
        <li><b>Open BG:</b> Open the saved background image in a separate window if it exists.</li>
        <li><b>Open Settings:</b> Load/reload the detector preset. Tune the values in the separate tuner GUI.</li>
        </ul>
        ...
        <h3>4. Run</h3>
        <ul>
        <li>Run tracking after setting scale, defining at least one arena, and selecting a single-bee settings preset. A static background is required only if the preset uses static mode.</li>
        <li><b>Per Number of Frames:</b> Speed option. 1 means detect every frame. 5 means real detection is run on frames 1, 5, 10, 15, ... . The skipped frames are still written to the coordinate files; raw is missing and filtered is Kalman prediction when available. Adaptive background is still updated on every frame.</li>
        </ul>
        """
    ...
    def create_control_window(self):
        ...
        self.btn_open_dict = QtWidgets.QPushButton("Open Settings")
        ...
        self.edit_aruco_dict_path = QtWidgets.QLineEdit()
        self.btn_open_aruco_dict = QtWidgets.QPushButton("Browse")
        
        self.chk_blob_detection = QtWidgets.QCheckBox("Enable Blob Detection (Bg Subtraction)")
        self.chk_blob_detection.setChecked(True)
        self.chk_blob_detection.setToolTip("If unchecked, bypasses background subtraction and relies entirely on ArUco Full-Frame detection.")
        self.chk_blob_detection.toggled.connect(self.on_blob_detection_toggled)
        ...
        self.spin_detect_every_n_frames = QtWidgets.QSpinBox(minimum=1, maximum=100000, value=1)
        self.spin_detect_every_n_frames.setToolTip(
            "Speed option. 1 = run detection every frame. "
            "5 = run detection on frames 1, 5, 10, 15, ...; skipped frames use Kalman prediction. "
            "Adaptive background is still updated on every frame."
        )
        ...
        aruco_row = QtWidgets.QHBoxLayout()
        aruco_row.addWidget(self.edit_aruco_dict_path)
        aruco_row.addWidget(self.btn_open_aruco_dict)
        setup_layout.addRow("Custom ArUco Dictionary:", aruco_row)

        dict_row = QtWidgets.QHBoxLayout()
        dict_row.addWidget(self.edit_dict_path)
        dict_row.addWidget(self.btn_open_dict)
        setup_layout.addRow("Detector Settings:", dict_row)
        setup_layout.addRow(self.chk_blob_detection)
        ...
        speed_group = QtWidgets.QGroupBox("Speed / Frame Skipping")
        speed_layout = QtWidgets.QFormLayout(speed_group)
        detect_every_row = QtWidgets.QHBoxLayout()
        detect_every_row.addWidget(self.spin_detect_every_n_frames)
        detect_every_row.addWidget(QtWidgets.QLabel("1 = every frame; 5 = frames 1, 5, 10, 15, ..."))
        speed_layout.addRow("Per Number of Frames:", detect_every_row)
        ...
        self.btn_open_aruco_dict.clicked.connect(self.open_aruco_dictionary)
        ...
        self.btn_open_aruco_dict.setEnabled(True)
        self.btn_help.setEnabled(True)
        self.btn_exit.setEnabled(True)
        self.edit_exp_name.setEnabled(True)
        self.on_blob_detection_toggled(self.chk_blob_detection.isChecked())
        return win

    def on_blob_detection_toggled(self, checked):
        # If checked (Bg subtraction activated), we use detector settings.
        # If unchecked, we use the custom dictionary.
        self.edit_dict_path.setEnabled(checked)
        self.btn_open_dict.setEnabled(checked)
        self.edit_aruco_dict_path.setEnabled(not checked)
        self.btn_open_aruco_dict.setEnabled(not checked)
    ...
    def set_controls_enabled(self, enabled):
        widgets = [
            ...
            self.spin_min_area,
            self.spin_max_area,
            self.spin_detect_every_n_frames,
    ...
    def open_aruco_dictionary(self):
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self.controls,
        ...
        if file_path:
            self.edit_aruco_dict_path.setText(file_path)

    def open_dictionary(self):
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self.controls,
            "Open Single-Bee Detector Settings",
            "",
            "Text / INI Files (*.txt *.ini);;All Files (*)"
        )
        if not file_path:
            return

        try:
            preset = load_settings_txt(file_path)
        except Exception as exc:
            QtWidgets.QMessageBox.critical(self.controls, "Settings Error", f"Could not load settings file:\n{exc}")
            return

        self.edit_dict_path.setText(file_path)
        self.settings["single_bee_settings_path"] = file_path

        # Mirror the area settings in the old quick area widgets so the user can
        # still see the tuned scale of the detector. The final tracker uses the
        # preset values directly.
        try:
            self.spin_min_area.setValue(int(round(float(preset.blob_filter.min_area))))
            self.spin_max_area.setValue(int(round(float(preset.blob_filter.max_area))))
        except Exception:
            pass

        mode = str(preset.preprocess.background_mode).lower()
        if mode == "static":
            QtWidgets.QMessageBox.information(
                self.controls,
                "Settings Loaded",
                "Settings loaded. This preset uses static background, so background.png must exist or be built before tracking.",
            )
        else:
            QtWidgets.QMessageBox.information(
                self.controls,
                "Settings Loaded",
                f"Settings loaded. Background mode: {mode}. No static background is required.",
            )
    ...
    def open_settings_dialog(self):
        ...
        if current:
            try:
                preset = load_settings_txt(current)
                self.settings["single_bee_settings_path"] = current
                self.spin_min_area.setValue(int(round(float(preset.blob_filter.min_area))))
                self.spin_max_area.setValue(int(round(float(preset.blob_filter.max_area))))
                QtWidgets.QMessageBox.information(self.controls, "Settings Reloaded", f"Reloaded:\n{current}")
                return
            except Exception as exc:
                QtWidgets.QMessageBox.warning(self.controls, "Reload Failed", f"Could not reload current settings file:\n{exc}")
    ...
    def run_analysis(self):
        ...
        settings_path = ""
        bg_path = None
        if self.chk_blob_detection.isChecked():
            settings_path = self.edit_dict_path.text().strip()
            if not settings_path:
                QtWidgets.QMessageBox.warning(self.controls, "Warning", "Please select the single-bee detector settings file saved by the tuner GUI.")
                return
            try:
                preset = load_settings_txt(settings_path)
            except Exception as exc:
                QtWidgets.QMessageBox.critical(self.controls, "Settings Error", f"Could not load detector settings:\n{exc}")
                return

            background_mode = str(preset.preprocess.background_mode).strip().lower()
            bg_path = self.background_path()
            if background_mode == "static":
                if bg_path is None or not bg_path.exists():
                    QtWidgets.QMessageBox.warning(
                        self.controls,
                        "Warning",
                        "The selected preset uses static background. Please build or provide background.png first.",
                    )
                    return
        else:
            aruco_path = self.edit_aruco_dict_path.text().strip()
            if not aruco_path:
                QtWidgets.QMessageBox.warning(self.controls, "Warning", "Please select a custom ArUco dictionary text file.")
                return
        ...
        self.settings["start_time_str"] = self.edit_start_time.text()
        self.settings["end_time_str"] = self.edit_end_time.text()
        ...
        self.settings["single_bee_settings_path"] = settings_path
        self.settings["aruco_dict_path"] = self.edit_aruco_dict_path.text()
        self.settings["background_path"] = str(bg_path) if bg_path is not None and bg_path.exists() else ""
        self.settings["tracking_method"] = "Single Bee Detector"
        ...
        self.settings["detect_every_n_frames"] = max(1, int(self.spin_detect_every_n_frames.value()))
        self.settings["blob_detection_enabled"] = self.chk_blob_detection.isChecked()

        # Write info/raw/filtered TXT files while tracking is running.
        # This protects long recordings from total data loss if the computer
        # reboots or the program stops before plots/videos are generated.
        self.settings["stream_output_realtime"] = True
        self.settings["stream_flush_interval"] = 100
        self.settings["stream_fsync_interval"] = 1000
    ...
    def save_output_files(self, output_data):
        """
        Coordinate TXT files are now normally written in real time by
        TrackingEngine.run_tracking(). This function should not overwrite them.
        It only falls back to writing coordinate files if realtime streaming was
        disabled or unavailable, then creates end-of-run plots/videos.
        """
        base_name = self.settings["exp_name"]
        engine = output_data.get("engine")

        if engine is not None and getattr(engine, "output_dir_path", None):
            output_dir = engine.output_dir_path
        else:
            parent_dir = self.settings.get("output_dir") or os.path.dirname(self.settings["video_path"])
            output_dir = os.path.join(parent_dir, f"{base_name}_output")
        ...
        raw_path = getattr(engine, "raw_coords_path", None) if engine is not None else None
        filt_path = getattr(engine, "filtered_coords_path", None) if engine is not None else None
        realtime_files_exist = bool(raw_path and filt_path and os.path.exists(raw_path) and os.path.exists(filt_path))

        if not realtime_files_exist:
            # Fallback path for older engines or debugging. New normal behavior is
            # realtime writing inside TrackingEngine.
            ...
            with open(meta_file_path, "w", encoding="utf-8") as f:
                ...
                f.write("# Inputs\n")
                f.write(f"single_bee_settings_path\t{self.settings.get('single_bee_settings_path', '')}\n")
                f.write(f"background_path\t{self.settings.get('background_path', '')}\n")
                f.write("tracking_method\tSingle Bee Detector\n")
                f.write(f"detect_every_n_frames\t{self.settings.get('detect_every_n_frames', 1)}\n\n")
                ...
                f.write("# Scale\n")
                f.write(f"pixel_to_mm\t{self.settings.get('pixel_to_mm', '')}\n\n")
            ...
            settings_path = self.settings.get("single_bee_settings_path", "")
            if settings_path and os.path.exists(settings_path):
                used_settings_path = os.path.join(output_dir, f"{base_name}_used_single_bee_settings.txt")
                try:
                    shutil.copyfile(settings_path, used_settings_path)
                except Exception:
                    pass
            ...
            raw_rows = []
            for frame_coords, frame_angles in zip(raw_coords, raw_angles):
                row = []
                for pos, ang in zip(frame_coords, frame_angles):
                    row.extend([str(pos[0]), str(pos[1]), f"{float(ang):.6f}"])
                raw_rows.append(row)

            filtered_rows = []
            for frame_coords, frame_angles in zip(filtered_coords, filtered_angles):
                row = []
                for pos, ang in zip(frame_coords, frame_angles):
                    row.extend([str(pos[0]), str(pos[1]), f"{float(ang):.6f}"])
