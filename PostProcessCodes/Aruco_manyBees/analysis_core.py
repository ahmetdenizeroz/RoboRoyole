...
def parse_info_file(info_filepath: str) -> Optional[dict]:
    info = {
        ...
        "tracking_method": "",
        "single_bee_settings_path": "",
        "single_bee_id": "ID_1",
        "background_mode": "",
        "adaptive_history_length": "",
        "adaptive_background_method": "",
    }
...
# The single-bee tracker deliberately writes ID_1_X / ID_1_Y / ID_1_Ang
# so this same parser works for old multi-marker outputs and new single-bee outputs.
...
def build_source_framewise_df(compare_df: pd.DataFrame, info_data: dict, source: str) -> pd.DataFrame:
    ...
    for shape_id, shape_data in arenas.items():
        ...
        df[f"inside_{shape_id}"] = np.nan
        df[f"dist_to_{shape_id}_center_mm"] = np.nan
        inside_mask = []
        dist_values = []
        for x, y, ok in zip(df["x"], df["y"], valid):
            if not ok:
                inside_mask.append(np.nan)
                dist_values.append(np.nan)
            else:
                inside_mask.append(1.0 if is_point_in_shape((x, y), shape_data) else 0.0)
                dist_values.append(math.hypot(x - center[0], y - center[1]) * pixel_to_mm)
        df[f"inside_{shape_id}"] = inside_mask
        df[f"dist_to_{shape_id}_center_mm"] = dist_values
    ...
    for bee_id, bee_df in df.groupby("bee_id", sort=False):
        ...
        for i in range(1, len(bee_df)):
            if valid_mask[i] and valid_mask[i - 1]:
                dist_px = math.hypot(x[i] - x[i - 1], y[i] - y[i - 1])
                speed[i] = dist_px * pixel_to_mm * fps
                angle_change[i] = wrap_angle_diff(ang[i], ang[i - 1])
        ...
        for stim_id in stimuli:
            ...
            for i in range(len(bee_df)):
                if np.isnan(inside[i]):
                    continue
                prev = inside[i - 1] if i > 0 else np.nan
                if i == 0 or np.isnan(prev):
                    entry[i] = 1.0 if inside[i] == 1.0 else 0.0
                    exit_[i] = 0.0
                else:
                    entry[i] = 1.0 if prev == 0.0 and inside[i] == 1.0 else 0.0
                    exit_[i] = 1.0 if prev == 1.0 and inside[i] == 0.0 else 0.0
...
def create_static_plots(bundle: AnalysisBundle, output_directory: str, scale_by_id: Optional[str] = None) -> dict:
    ...
    for source, binned_df in bundle.binned_by_source.items():
        ...
        y_limits = {}
        if scale_by_id:
            reference_binned = binned_df[binned_df["BeeID"] == scale_by_id]
            if not reference_binned.empty:
                speed_vals = reference_binned.get("AvgSpeed_mm_s")
                if speed_vals is not None and speed_vals.notna().any():
                    y_limits["speed"] = (0, speed_vals.max() * 1.1)

                dist_arena_vals = []
                for arena_id in arenas:
                    col = f"AvgDistArenaCenter_{arena_id}_mm"
                    if col in reference_binned.columns and reference_binned[col].notna().any():
                        dist_arena_vals.append(reference_binned[col].max())
                if dist_arena_vals:
                    y_limits["dist_arena"] = (0, max(dist_arena_vals) * 1.1)

                stim_dur_vals = []
                for stim_id in stimuli:
                    col = f"Duration_in_Stim_{stim_id}_s"
                    if col in reference_binned.columns and reference_binned[col].notna().any():
                        stim_dur_vals.append(reference_binned[col].max())
                if stim_dur_vals:
                    y_limits["stim_duration"] = (0, max(stim_dur_vals) * 1.1)

                stim_dist_vals = []
                for stim_id in stimuli:
                    col = f"AvgDistStimCenter_{stim_id}_mm"
                    if col in reference_binned.columns and reference_binned[col].notna().any():
                        stim_dist_vals.append(reference_binned[col].max())
                if stim_dist_vals:
                    y_limits["stim_dist"] = (0, max(stim_dist_vals) * 1.1)

                stim_entries_vals = []
                for stim_id in stimuli:
                    col1 = f"Entries_to_Stim_{stim_id}"
                    col2 = f"Exits_from_Stim_{stim_id}"
                    if col1 in reference_binned.columns and reference_binned[col1].notna().any():
                        stim_entries_vals.append(reference_binned[col1].max())
                    if col2 in reference_binned.columns and reference_binned[col2].notna().any():
                        stim_entries_vals.append(reference_binned[col2].max())
                if stim_entries_vals:
                    y_limits["stim_entries"] = (0, max(stim_entries_vals) * 1.1)
            ...
            try:
                ...
                if subplot_specs:
                    ...
                    for ax, plot_type in zip(axes, subplot_specs):
                        ...
                        if scale_by_id and plot_type in y_limits:
                            ax.set_ylim(y_limits[plot_type])
        ...
        # --- Summary Bar Plot for Multi-Bee ---
        if len(bundle.bee_ids) > 1:
            try:
                ...
                for bee_id in bundle.bee_ids:
                    bee_binned = binned_df[binned_df["BeeID"] == bee_id]
                    if bee_binned.empty:
                        avg_speeds.append(0)
                        avg_dist_arena.append(0)
                        stim_durations.append(0)
                        avg_dist_stim.append(0)
                        total_transitions.append(0)
                        continue
                    
                    val_speed = bee_binned["AvgSpeed_mm_s"].mean(skipna=True) if "AvgSpeed_mm_s" in bee_binned.columns else np.nan
                    avg_speeds.append(val_speed if pd.notna(val_speed) else 0)
                    
                    arena_dists = []
                    for a in arenas:
                        col = f"AvgDistArenaCenter_{a}_mm"
                        if col in bee_binned.columns:
                            m = bee_binned[col].mean(skipna=True)
                            if pd.notna(m):
                                arena_dists.append(m)
                    avg_dist_arena.append(np.mean(arena_dists) if arena_dists else 0)
                    
                    stim_durs = []
                    for s in stimuli:
                        col = f"Duration_in_Stim_{s}_s"
                        if col in bee_binned.columns:
                            su = bee_binned[col].sum(skipna=True)
                            if pd.notna(su):
                                stim_durs.append(su)
                    stim_durations.append(np.sum(stim_durs) if stim_durs else 0)
                    
                    stim_dists = []
                    for s in stimuli:
                        col = f"AvgDistStimCenter_{s}_mm"
                        if col in bee_binned.columns:
                            m = bee_binned[col].mean(skipna=True)
                            if pd.notna(m):
                                stim_dists.append(m)
                    avg_dist_stim.append(np.mean(stim_dists) if stim_dists else 0)
                    
                    entries_exits = 0
                    for s in stimuli:
                        col1 = f"Entries_to_Stim_{s}"
                        col2 = f"Exits_from_Stim_{s}"
                        if col1 in bee_binned.columns:
                            entries_exits += bee_binned[col1].sum(skipna=True)
                        if col2 in bee_binned.columns:
                            entries_exits += bee_binned[col2].sum(skipna=True)
                    total_transitions.append(entries_exits)
                
                x_pos = np.arange(len(bundle.bee_ids))
                
                axes[0].bar(x_pos, avg_speeds, color='skyblue')
                axes[0].set_ylabel("mm/s")
                axes[0].set_title("Overall Average Speed")
                
                axes[1].bar(x_pos, avg_dist_arena, color='lightgreen')
                axes[1].set_ylabel("mm")
                axes[1].set_title("Overall Average Distance to Arena Center")
                
                axes[2].bar(x_pos, stim_durations, color='salmon')
                axes[2].set_ylabel("s")
                axes[2].set_title("Total Duration in Stimulus")
                
                axes[3].bar(x_pos, avg_dist_stim, color='orange')
                axes[3].set_ylabel("mm")
                axes[3].set_title("Overall Average Distance to Stimulus Center")
                
                axes[4].bar(x_pos, total_transitions, color='plum')
                axes[4].set_ylabel("count")
                axes[4].set_title("Total Stimulus Entries + Exits")
                
                axes[4].set_xticks(x_pos)
                axes[4].set_xticklabels(bundle.bee_ids, rotation=45, ha='right')
                
                for ax in axes:
                    ax.grid(True, linestyle=":", alpha=0.6, axis='y')
                
                fig.suptitle(f"{bundle.experiment_name} | Multi-Bee Summary | {source}")
                plt.tight_layout(rect=[0, 0.03, 1, 0.97])
                summary_path = source_dir / "multi_bee_summary_bar_plots.png"
                fig.savefig(summary_path, dpi=120, bbox_inches="tight")
                plt.close(fig)
                saved[f"summary_bar_{source}"] = str(summary_path)
            except Exception as exc:
                failures.append(f"Summary bar plot ({source}): {exc}")
            finally:
                plt.close("all")
