...
class ArucoSettings:
    enabled: bool = False
    dict_path: str = "3x3_5.txt"
    detect_mode: str = "arena"
    max_hamming: int = 1
    max_border_errors: int = 4
    crop_padding: int = 20
...
class SingleBeeDetectorSettings:
    ...
    aruco: ArucoSettings = field(default_factory=ArucoSettings)
...
class CandidateBlob:
    ...
    aruco_corners: Optional[np.ndarray] = None
    aruco_angle: Optional[float] = None
...
class SingleBeeDetectionResult:
    ...
    message: str = ""
    selected_candidates: List[CandidateBlob] = field(default_factory=list)
...
def settings_to_config(settings: SingleBeeDetectorSettings) -> configparser.ConfigParser:
    ...
    cfg["candidate_selection"] = {
        ...
        "max_bees": str(settings.candidate_selection.max_bees),
    ...
    cfg["aruco"] = {
        "enabled": _bool_to_str(settings.aruco.enabled),
        "dict_path": settings.aruco.dict_path,
        "detect_mode": getattr(settings.aruco, "detect_mode", "arena"),
        "max_hamming": str(settings.aruco.max_hamming),
        "max_border_errors": str(settings.aruco.max_border_errors),
        "crop_padding": str(settings.aruco.crop_padding),
    }
...
def config_to_settings(cfg: configparser.ConfigParser) -> SingleBeeDetectorSettings:
    ...
    c.max_bees = _get_int(cfg, "candidate_selection", "max_bees", c.max_bees)
    ...
    a = d.aruco
    a.enabled = _get_bool(cfg, "aruco", "enabled", a.enabled)
    a.dict_path = _get_str(cfg, "aruco", "dict_path", a.dict_path)
    a.detect_mode = _get_str(cfg, "aruco", "detect_mode", getattr(a, "detect_mode", "arena"))
    a.max_hamming = _get_int(cfg, "aruco", "max_hamming", a.max_hamming)
    a.max_border_errors = _get_int(cfg, "aruco", "max_border_errors", a.max_border_errors)
    a.crop_padding = _get_int(cfg, "aruco", "crop_padding", a.crop_padding)
...
class SingleBeeDetector:
    ...
    def detect(
        ...
        skip_blob_detection = self.settings.aruco.enabled and getattr(self.settings.aruco, 'detect_mode', 'arena') == "full_frame"

        if not skip_blob_detection:
            ...
            selected_list = self.select_candidates(candidates, previous_position=prev_for_candidates)

            if selected_list and scale < 0.999 and bool(s.refine_on_original):
                for idx, sel_c in enumerate(selected_list):
                    ...
                    if refined is not None:
                        refined.index = sel_c.index
                        refined.score = sel_c.score
                        refined.selected = False
                        selected_list[idx] = refined
            ...
            channel_full = extract_channel(frame_crop, self.settings.preprocess.channel)
            diff_full = np.zeros_like(channel_full)
            bg_used_full = np.zeros_like(channel_full)
            channel_proc = channel_full
            diff_proc = diff_full
            binary = diff_full
            cleaned = diff_full
        ...
        if not selected_list and not skip_blob_detection:
        ...
        for sel_c in selected_list:
            sel_c.selected = True
            
        if self.settings.aruco.enabled:
            if self.aruco_detector is None or self._last_aruco_dict != self.settings.aruco.dict_path:
                try:
                    import sys
                    from pathlib import Path
                    ...
                    dict_file_path = self.settings.aruco.dict_path
                    if not Path(dict_file_path).is_absolute():
                        dict_file_path = str(Path(script_dir) / dict_file_path)
                        
                    self.aruco_detector = BeeDetector(dict_file_path)
                    self._last_aruco_dict = self.settings.aruco.dict_path
                except Exception as e:
                    print(f"Failed to load ArUco detector: {e}")
                    self.aruco_detector = None

            if self.aruco_detector is not None:
                self.aruco_detector.set_decode_settings(
                    max_hamming=self.settings.aruco.max_hamming,
                    max_border_errors=self.settings.aruco.max_border_errors
                )
                
                ch_img = extract_channel(frame_bgr, self.settings.preprocess.channel)
                new_known_positions = []
                unidentified_blobs = []
                if getattr(self.settings.aruco, 'detect_mode', 'arena') == "full_frame":
                    work_img_for_aruco = ch_img
                    offset_x, offset_y = 0, 0
                else:
                    if crop_bbox is not None:
                        bx, by, bw, bh = crop_bbox
                        work_img_for_aruco = ch_img[by:by+bh, bx:bx+bw]
                ...
                try:
                    aruco_results = self.aruco_detector.detect(work_img_for_aruco)
                    detections = aruco_results.get("detections", [])
                except Exception:
                    detections = []
                
                used_detections = set()
                
                # Assign to existing candidates
                for cand in selected_list:
                    ...
                    for idx, det in enumerate(detections):
                        if idx in used_detections: continue
                        m_center = det["marker_corners"].mean(axis=0) + [offset_x, offset_y]
                        dist = math.hypot(cand.centroid[0] - m_center[0], cand.centroid[1] - m_center[1])
                        if dist < best_dist and dist < 100.0:  # Distance threshold
                            best_dist = dist
                            best_match = det
                            best_idx = idx
                            
                    if best_match is not None:
                        cand.aruco_id = best_match["id"]
                        cand.aruco_angle = best_match["angle_deg"]
                        cand.aruco_corners = best_match["marker_corners"] + [offset_x, offset_y]
                        new_known_positions.append({'id': cand.aruco_id, 'centroid': cand.centroid})
                        used_detections.add(best_idx)
                    else:
                        unidentified_blobs.append(cand)

                # Create synthetic blobs for unassigned ArUco detections
                for idx, det in enumerate(detections):
                    if idx not in used_detections:
                        corners_global = det["marker_corners"] + [offset_x, offset_y]
                        m_center = corners_global.mean(axis=0)
                        min_x, min_y = np.min(corners_global, axis=0).astype(int)
                        max_x, max_y = np.max(corners_global, axis=0).astype(int)
                        
                        synth_cand = CandidateBlob(
                            index=len(selected_list),
                            contour=corners_global.astype(np.int32).reshape(-1, 1, 2),
                            area=cv2.contourArea(corners_global.astype(np.float32)),
                            centroid=(float(m_center[0]), float(m_center[1])),
                            ...
                            extent=1.0,
                            aspect_ratio=1.0,
                            major_axis_angle_deg=det["angle_deg"],
                            major_axis_vector=(math.cos(math.radians(det["angle_deg"])), math.sin(math.radians(det["angle_deg"]))),
                            minor_axis_vector=(-math.sin(math.radians(det["angle_deg"])), math.cos(math.radians(det["angle_deg"]))),
                            major_axis_endpoints=((0,0), (0,0)),
                            minor_axis_endpoints=((0,0), (0,0)),
                        )
                        synth_cand.aruco_id = det["id"]
                        synth_cand.aruco_angle = det["angle_deg"]
                        synth_cand.aruco_corners = corners_global
                        synth_cand.selected = True
                        
                        selected_list.append(synth_cand)
                        new_known_positions.append({'id': synth_cand.aruco_id, 'centroid': synth_cand.centroid})

                    
                max_dist = float(self.settings.candidate_selection.max_jump_px)
                if max_dist <= 0.0:
                    max_dist = 50.0
                    
                used_ids = {p['id'] for p in new_known_positions}
                
                for cand in unidentified_blobs:
                    best_dist = max_dist
                    best_id = None
                    for p in self._last_known_positions:
                        if p['id'] in used_ids:
                            continue
                        dist = math.hypot(cand.centroid[0] - p['centroid'][0], cand.centroid[1] - p['centroid'][1])
                        if dist < best_dist:
                            best_dist = dist
                            best_id = p['id']
                            
                    if best_id is not None:
                        cand.aruco_id = best_id
                        used_ids.add(best_id)
                        new_known_positions.append({'id': best_id, 'centroid': cand.centroid})
                        
                self._last_known_positions = new_known_positions
        ...
        first = selected_list[0]
        return SingleBeeDetectionResult(
            detected=True,
            state="detected",
            x=float(first.centroid[0]),
            y=float(first.centroid[1]),
            angle_deg=float(first.major_axis_angle_deg),
            area=float(first.area),
            bbox=first.bbox,
            selected_candidate=first,
            selected_index=first.index,
            candidates=candidates,
            debug_images=DebugImages(
                channel=channel_proc,
                difference=diff_proc,
            ...
            message=f"Detected {len(selected_list)} bees.",
    ...
    def select_candidate(
        ...
        res = self.select_candidates(candidates, previous_position)
    ...
    def select_candidates(
        ...
        if method == "closest_to_previous":
            if previous_position is not None:
                valid.sort(key=lambda c: ((c.distance_to_previous or 1e12), -c.area))
            elif not bool(sel.allow_largest_if_no_previous):
                return []
            else:
                valid.sort(key=lambda c: -c.area)
        else:
            valid.sort(key=lambda c: (-(c.score if c.score is not None else -1e12), -c.area))

        max_bees = getattr(sel, 'max_bees', 1)
        return valid[:max_bees]
...
def draw_detection_overlay(
    ...
    *,
    ...
    if result.detected and getattr(result, "selected_candidates", None):
        for i, c in enumerate(result.selected_candidates):
            cx, cy = int(round(c.centroid[0])), int(round(c.centroid[1]))
            cv2.circle(out, (cx, cy), 6, (0, 255, 0), 2)

            id_text = f"ID:{c.aruco_id}" if getattr(c, 'aruco_id', None) is not None else f"bee{i}"

            # Draw label floating above the bee circle
            label_y = max(cy - 12, 15)
            (tw, th), baseline = cv2.getTextSize(id_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
            label_x = max(0, min(cx - tw // 2, out.shape[1] - tw - 2))
            cv2.rectangle(out, (label_x - 2, label_y - th - 4), (label_x + tw + 2, label_y + baseline), (0, 0, 0), -1)
            cv2.putText(out, id_text, (label_x, label_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)

            # Also write compact status in top-left corner
            cv2.putText(
                out,
                f"{id_text}: x={c.centroid[0]:.1f}, y={c.centroid[1]:.1f}, ang={c.major_axis_angle_deg:.1f}",
                (10, 25 + i * 22),
            ...
            if getattr(c, 'aruco_corners', None) is not None:
                corners = c.aruco_corners.astype(np.int32).reshape((-1, 1, 2))
                cv2.polylines(out, [corners], True, (255, 165, 0), 2)
                cv2.circle(out, tuple(corners[0][0]), 4, (0, 0, 255), -1)
    elif result.detected and result.selected_candidate is not None:
...
def summarize_result(result: SingleBeeDetectionResult, max_candidates: int = 12) -> str:
    ...
    if result.detected:
        if getattr(result, "selected_candidates", None):
            for i, c in enumerate(result.selected_candidates):
                id_text = f" [ArUco ID={c.aruco_id}]" if getattr(c, 'aruco_id', None) is not None else ""
                lines.append(
                    f"Selected {i}{id_text}: #{c.index}  x={c.centroid[0]:.2f}, y={c.centroid[1]:.2f}, "
                    f"angle={c.major_axis_angle_deg:.2f} deg, area={c.area:.1f}, bbox={c.bbox}"
                )
        else:
            lines.append(
