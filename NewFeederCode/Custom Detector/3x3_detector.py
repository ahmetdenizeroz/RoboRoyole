...
CELL_PIXELS = 24  # increased from 24

# Decode tolerance
MAX_HAMMING = 0
MAX_BORDER_ERRORS = 10   # relaxed from 4
...
def select_roi_from_frame(frame):
    roi = cv2.selectROI(
        "Select ROI around ID0 marker",
        frame,
        fromCenter=False,
        showCrosshair=True
    )
    cv2.destroyWindow("Select ROI around ID0 marker")
...
def find_square_candidates(red_channel, roi=None):
    ...
    _, bw_inv = cv2.threshold(red_channel, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
...
def main():
    ...
    cap = cv2.VideoCapture("manybeestwo.mp4")
    if not cap.isOpened():
