...
def load_custom_markers_from_txt(txt_path):
    ...
    marker_size_match = re.search(r"marker_size\s*=\s*(\d+)x(\d+)", text)
    if not marker_size_match:
        raise ValueError("Could not find marker_size in txt file.")

    rows = int(marker_size_match.group(1))
    cols = int(marker_size_match.group(2))
    if rows != cols:
        raise ValueError("Only square markers are supported.")
    if rows != MARKER_SIZE:
        raise ValueError(
            f"TXT says marker size is {rows}x{cols}, but code expects {MARKER_SIZE}x{MARKER_SIZE}."
        )
    ...
    for match in pattern.finditer(text):
        ...
        rows_text = [match.group(2), match.group(3), match.group(4)]

        arr = []
        for row_text in rows_text:
            vals = [int(x) for x in row_text.strip().split()]
            if len(vals) != MARKER_SIZE:
                raise ValueError(f"Invalid row length in marker ID {marker_id}.")
            arr.append(vals)

        markers[marker_id] = np.array(arr, dtype=np.uint8)
...
def select_roi_from_frame(frame):
    roi = cv2.selectROI(
        "Select ROI around marker",
        frame,
        fromCenter=False,
        showCrosshair=True
    )
    cv2.destroyWindow("Select ROI around marker")
...
def find_square_candidates(red_channel, roi=None, debug=False):
    """
    Finds square-ish dark candidates from the red channel.
    Keeps only candidates whose center is inside ROI.

    Debug windows:
    - debug_candidate_binary
    - debug_candidate_overlay
    """
    ...
    for cnt in contours:
        ...
        raw_quads.append(ordered)

    quads = deduplicate_quads(raw_quads)

    if debug:
        cv2.imshow("debug_candidate_binary", bw_inv)

        overlay = cv2.cvtColor(bw_inv, cv2.COLOR_GRAY2BGR)
        ...
        for i, q in enumerate(raw_quads):
            ...
            cv2.polylines(overlay, [qq], True, (0, 165, 255), 1, cv2.LINE_AA)
            c = np.mean(q, axis=0).astype(int)
            cv2.putText(
                overlay,
                f"R{i}",
                (int(c[0]) + 4, int(c[1]) - 4),
        ...
        # final quads in yellow
        for i, q in enumerate(quads):
            qq = np.asarray(q, dtype=np.int32).reshape(4, 1, 2)
            cv2.polylines(overlay, [qq], True, (0, 255, 255), 2, cv2.LINE_AA)
            c = np.mean(q, axis=0).astype(int)
            cv2.circle(overlay, tuple(c), 3, (0, 0, 255), -1)
            cv2.putText(
                overlay,
                f"C{i}",
                (int(c[0]) + 6, int(c[1]) - 6),
        ...
        cv2.imshow("debug_candidate_overlay", overlay)
        cv2.waitKey(1)
...
def main():
    try:
        custom_markers = load_custom_markers_from_txt(TXT_PATH)
    ...
    cap = cv2.VideoCapture("feed_output_20260415_133807_1920x1080.mp4")
    if not cap.isOpened():
    ...
    while True:
        ...
        quads = find_square_candidates(red, roi=roi, debug=SHOW_CANDIDATE_DEBUG)
        ...
        cv2.imshow("Custom 3x3 Marker Detection", vis)
