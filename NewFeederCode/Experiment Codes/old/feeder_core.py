        ...
        if not self.is_feeding and (self.frames_in_zone_counter >= entry_frames_target):
            if not self.arduino_healthy:
                if not self.auto_feed_blocked_until_exit:
                    self.status_updated.emit("Auto-feed blocked: Arduino unavailable/unhealthy.")
                    self._log_to_file("Auto-feed blocked: Arduino unavailable/unhealthy.")
                    self.log_pump_event("AUTO_FEED_BLOCKED_ARDUINO_UNAVAILABLE", current_tag_id)
